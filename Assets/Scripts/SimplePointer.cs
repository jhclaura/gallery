//====================================================================================
// Adopted from VRTK_SimplePointer.cs for Gvr
//
...
public class SimplePointer : MonoBehaviour
{
	...
	//
	Transform pointerContactTarget = null;
	Vector3 destinationPosition;
	float pointerContactDistance = 0f;

	//
	public FlyerManagerMobile flyerManagerMobile;
    ...
    void Update()
    {
        ...
        if (pointer.gameObject.activeSelf)
        {
			...
			//
			if(rayHit){
				flyerManagerMobile.OnObjectHover( pointerCollidedWith );
			}
        }
    }
    ...
    private float GetPointerBeamLength(bool hasRayHit, RaycastHit collidedWith)
    {
        ...
        if (hasRayHit)
        {
			...
			pointerMaterial.color = pointerHitColor;
			SetPointerMaterial();
