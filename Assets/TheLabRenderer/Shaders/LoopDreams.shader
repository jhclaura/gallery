...
#include "vr_fog.cginc"
	...
	PS_INPUT MainVs(VS_INPUT i)
	{
		...
		o.vPositionOs.xyz = i.vPositionOs.xyz;
...
#define g_flDetailNormalScale _DetailNormalMapScale
	...
	)
	{
		...
		// Albedo //
		//--------//
		float4 color1 = float4(0.0, 0.0, 1.0, 1.0);
		float4 color2 = float4(1.0, 0.0, 1.0, 1.0);
		float time = _Time.x * 100;
		float2 p = -1.0 + 2 * i.vPositionOs.yz * 100;
		p.x *= .2;
		float PI = 3.14159;
		...
		float4 vAlbedoTexel = lerp(color1, color2, c2);
		// float4 vAlbedoTexel = float4(c1, c2, c3, 1.0);
