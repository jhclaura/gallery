	...
	//		gameObject.GetComponent<Camera>().fieldOfView = 100;
		...
		public void RenderVideo()
		{
		 ...
		 public void SaveScreenshot(){
			...
			if (ImageFormatType == VRFormatList.JPG){
				...
				string filePath = string.Format("{0}/"+_prefix+"{1:D05}"+ timestamp +".jpg", Folder, (Time.frameCount - StartFrame - 1));
				formatString = ".jpg\"";
		 		Texture2D screenShot = GetScreenshot(true);
				StartCoroutine (SaveFileJPG (filePath, screenShot));
				}
				...
				if (captureType == VRCaptureList.StillImage) timestamp = System.DateTime.Now.Ticks + "Screenshot";
				string filePath = string.Format("{0}/"+_prefix+"{1:D05}"+ timestamp +".png", Folder, (Time.frameCount - StartFrame - 1));
				formatString = ".png\"";
				StartCoroutine (SaveFilePNG (filePath, screenShot));
				}
			}
	

		private static IEnumerator SaveFileJPG(string filePathd, Texture2D SShot){
			yield return null;
			byte[] bytes = SShot.EncodeToJPG(100);
		...
		public Texture2D GetScreenshot(bool eye){
			...
			if (panoramaType == VRModeList.EquidistantStereo || panoramaType == VRModeList.EquidistantStereoSBS){
				...
				if(panoramaType == VRModeList.EquidistantStereo) rt = RenderTexture.GetTemporary (resolution, resolutionH/2, 0);
				if(panoramaType == VRModeList.EquidistantStereoSBS) rt = RenderTexture.GetTemporary (resolution/2, resolutionH, 0);
			}

			else rt = RenderTexture.GetTemporary (resolution, resolutionH, 0);
			...
			if (panoramaType == VRModeList.EquidistantStereo || panoramaType == VRModeList.EquidistantStereoSBS){
				...
				if(panoramaType == VRModeList.EquidistantStereoSBS) screenShot.ReadPixels(new Rect(0, 0, resolution, resolutionH), 0, 0);
		...
		public void VideoRenderPrepare () {
			...
			unfilteredRt = new RenderTexture(resolution*4 / 32 * renderQuality, resolutionH * 4 / 32 * renderQuality, 0, RenderTextureFormat.DefaultHDR, RenderTextureReadWrite.Default);
		...
		public void SaveScreenshotVideo(){
			...
			if (ImageFormatType == VRFormatList.JPG)
			{
				byte[] bytes = screenShot.EncodeToJPG(100);
								string filePath = string.Format("{0}/"+_prefix+"{1:D05}.jpg", Folder, (Time.frameCount - StartFrame - 1));
				formatString = ".jpg\"";
			...
			else 
			{
				byte[] bytes = screenShot.EncodeToPNG();
								string filePath = string.Format("{0}/"+_prefix+"{1:D05}.png", Folder, (Time.frameCount - StartFrame -1 ));
				formatString = ".png\"";
				File.WriteAllBytes(filePath, bytes);
			}
...
//						float backDiff = rl - rr;
	...
	//		UInt16 two = 2;
		...
		public void PreparePano () {
			...
			else renderPanorama = (GameObject)Instantiate(Resources.Load("360Unwrapped"));
